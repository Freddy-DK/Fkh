    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          command = ["/bin/bash", "-c"]
          args = [
            "echo 'deb [arch=amd64 trusted=yes] https://packages.microsoft.com/ubuntu/22.04/mssql-server-2022 jammy main' > /etc/apt/sources.list.d/mssql-server-2022.list && apt-get update && apt-get install -y mssql-server-fts && exec /opt/mssql/bin/sqlservr"
          ]
