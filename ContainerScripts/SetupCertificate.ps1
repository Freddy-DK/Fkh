...
if ($containerSasUrl -and $env:pfxCertificatePassword) {
    ...
    try {
        ...
        if ($_.Exception.Response.StatusCode.value__ -eq 404) {
            Write-Host "No certificate found in blob storage (will create new)."
        }
        else {
            Write-Host "Warning: Could not download certificate from blob storage: $($_.Exception.Message)"
        }
    }
}

if ($certificateFromBlob) {
    Write-Host "Using certificate from blob storage"
    $cert = New-Object System.Security.Cryptography.X509Certificates.X509Certificate2($certificatePfxFile, $CertificatePfxPassword)
...
if ($containerSasUrl -and (Test-Path $certificatePfxFile)) {
    ...
    try {
        ...
        Write-Host "Certificate uploaded to blob storage."
    }
    catch {
        Write-Host "Warning: Could not upload certificate to blob storage: $($_.Exception.Message)"
    }
}

} # end else (certificateFromBlob)
